-- ============================================
-- MAO 2026 - SCRIPTS SQL CORREGIDOS PARA SUPABASE
-- Este script es CORRECTO para PostgreSQL 15+
-- ============================================

-- PASO 1: INSERTAR LICENCIA
DELETE FROM licenses WHERE school_code = 'MAO-2026';
...
DELETE FROM users WHERE auth_code IN ('SADMIN-K9X2-7M4L', 'ADMIN-5C1P-9Q3R', 'COMPRA-8N6T-2Y5W', 'FINAN-4D7B-1S9Z', 'GEREN-3H8K-6F2V', 'PROFE-2L5G-9C4X', 'AUDIT-7P1T-8B6E');
...
DELETE FROM inventory_items WHERE item_code IN ('LAP-001', 'LAP-002', 'HOJ-001', 'HOJ-002', 'ESC-001', 'ESC-002', 'SIL-001', 'SIL-002', 'MAR-001', 'DES-001', 'PER-001', 'GRA-001', 'CAR-001', 'GUA-001', 'LIM-001');
...
DELETE FROM suppliers WHERE name IN ('Distribuidora ABC', 'Papelería Nacional', 'Muebles Express', 'Ferretería Central', 'Importadora Global');
...
-- PASO 5: INSERTAR REQUISICIONES (5)
DELETE FROM requisitions WHERE requisition_number LIKE 'REQ-2026-%';
...
SELECT 
  ...
  'REQ-2026-0001',
  (SELECT id FROM users WHERE auth_code = 'PROFE-2L5G-9C4X' LIMIT 1),
  'pendiente',
  'alta',
...
DELETE FROM purchase_requests WHERE request_number LIKE 'SOL-2026-%';
...
DELETE FROM purchase_orders WHERE order_number LIKE 'ORD-2026-%';
...
-- ✅ LISTO - Todos los datos se crearon exitosamente
