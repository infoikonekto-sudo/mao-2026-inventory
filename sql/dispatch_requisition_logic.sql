-- ============================================================
-- LÓGICA DE DESPACHO DE REQUISICIONES
-- Valida stock y procesa la salida de inventario de forma atómica
-- ============================================================

-- Asegurar columnas necesarias en `inventory_movements`
DO $$
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM information_schema.columns
		WHERE table_schema='public' AND table_name='inventory_movements' AND column_name='item_code'
	) THEN
		ALTER TABLE public.inventory_movements ADD COLUMN item_code text;
	END IF;
    ...
    IF NOT EXISTS (
      ...
      ALTER TABLE public.inventory_movements ADD COLUMN inventory_item_id uuid;
    END IF;
END $$;

-- 1) FUNCIÓN: Validar y procesar despacho de requisición
-- Retorna: 
--   - SUCCESS: si la requisición fue despachada exitosamente
--   - ERROR: si no hay stock suficiente (incluye detalles del déficit)
...
-- 2) ACTUALIZAR ESTADIOS DE REQUISICIÓN
-- Agregar 'entregado' a los valores permitidos si aún no existe
DO $$
BEGIN
...
EXCEPTION WHEN OTHERS THEN
	RAISE NOTICE 'Constraint actualizado o ya existe: %', SQLERRM;
END $$;

-- 3) CREAR VISTA DE DESPACHO
-- Muestra el resumen de lo que se puede despachar
DROP VIEW IF EXISTS public.vw_requisition_dispatch_summary CASCADE;
CREATE VIEW public.vw_requisition_dispatch_summary AS
SELECT 
	r.id,
...
-- 4) CREAR VISTA DE DETALLES DE DESPACHO
-- Muestra los items específicos de una requisición
DROP VIEW IF EXISTS public.vw_requisition_dispatch_details CASCADE;
CREATE VIEW public.vw_requisition_dispatch_details AS
SELECT 
